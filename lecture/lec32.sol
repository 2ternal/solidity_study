...
contract lec32 {
    /*
Payable 
Payable은 이더/토큰과 상호작용시 필요한 키워드
즉, send, trnafer, call을 이용하여, 이더를 보낼때 Payable이라는 키워드가 필요하다.
이 Payable은 주로 함수,주소,생성자에 붙여서 사용된다.

msg.value
msg.value는 송금보낼 코인의 값
이더를 보내는 3가지 
    ...
    3.call : 가변적인 gas 소비 (gas값 지정 가능), 성공여부를 true 또는 false로 리턴
             재진입(re-entrancy) 공격 위험성 있음, 2019년 12월 이후 gas의 가격이 올라서 call 사용을 추천. 
    */
    ...
    function transferNow(address payable _to) public payable{
        _to.transfer(msg.value);    //transfer자체 내에 error handler가 있음
        emit howMuch(msg.value);
    }
