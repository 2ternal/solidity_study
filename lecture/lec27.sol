...
-try/catch문 안에서, assert/revert/require을 통해 에러가 난다면 catch는 에러를 잡지를 못하고, 
	개발자가 의도한것으로 알고 정상적으로 프로그램을 끝낸다.
...
외부 스마트 컨트랙을 생성 할 때 : 다른 스마트컨트랙을 인스턴스화하여 생성 할 때 씀
...
contract runner{
    ...
    function playTryCatch(uint256 _num1, uint256 _num2) public returns(uint256,bool){
        ...
        try mathInstance.division(_num1, _num2) returns(uint256 value){ //외부 스마트컨트랙 함수를 부를 때
            return(value,true);
        ...
        } catch Panic(uint256 _errorCode) { //0.8.1이상 버전에서만
